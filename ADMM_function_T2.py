...
def VPDADMM_noearlystopping(A, Y, X, V, N, Adjacent_Matrix, alpha, c, gamma, MAX_ITER, eta, USTp, Umin):
    iter_num = 10000
    m = np.size(A, 0)
    T = np.size(A, 1)
    # print K
    ...
    for k in range(MAX_ITER):
        ...
        print("end of iteration %d, avr_com_mse: %10.4f, avr_loc_mse: %10.4f, avr_v:  %10.4f\n"
    ...
    return avr_com_mse, avr_loc_mse, avr_v  # [MAX_ITER-1]


def VPDADMM_nohardmapping(A, Y, X, V, N, Adjacent_Matrix, alpha, c, gamma, MAX_ITER, eta, USTp, Umin):
    iter_num = 10000
    m = np.size(A, 0)
    T = np.size(A, 1)
    # print K
...
def ADMM_VPD_JSM1_ind(A, Y, X, V, N, Adjacent_Matrix, alpha, c,
    ...
    return avr_com_mse, avr_loc_mse, avr_v # [MAX_ITER-1]  #, mse_total, cserr, maxiter
