...
# coding=utf-8
import numpy as np
from dmvcs.optimization import update_l1_cvx as upd
from dmvcs.optimization import update_l1_cvx_jsm1 as updJSM1
from dmvcs.optimization import update_l1_cvx_error_correction as updEC
from dmvcs.inference import view_inference as iv
import copy
...
def baseline_Cint(A, Y, X, V, N, M_list, gamma):
    T = np.size(A, 1)
    ...
    loss=np.zeros(N)
    for i in range(N):
        C_i = C[sum(M_list[0:i]):sum(M_list[0:i+1]), :]
        Y_i = Y[sum(M_list[0:i]):sum(M_list[0:i+1])]
        V_i = V[i * T: (i + 1) * T]
...
def baseline_JSM1(A, Y, X, V, N, Adjacent_Matrix, c, rho, gamma1, gamma2, gamma3, MAX_ITER):
    ...
    time_alliter = np.zeros(MAX_ITER)
    ...
    for k in range(MAX_ITER):
        ...
        for i in range(N):
            ...
            time_end = time.time()
            time_allnode[i] = time_end-time_start
            x_local = e_e[:, i]
        ...
        time_alliter[k] = np.max(time_allnode)
        ...
        print("end of iteration %d, avr_com_mse: %10.4f, avr_loc_mse: %10.4f, time:  %10.4f\n"
            % (k, avr_com_mse[k], avr_loc_mse[k], np.sum(time_alliter[0: k])))
    ...
    return avr_com_mse, avr_loc_mse, time_alliter  #[MAX_ITER-1] #, mse_total, cserr, maxiter
...
def decentral_l1_VR_penalty_c_ind_hard(A, Y, X, V, N, Adjacent_Matrix, alpha, c, gamma, MAX_ITER, eta, USTp, Umin):
    ...
    return avr_com_mse, avr_loc_mse, time_alliter
