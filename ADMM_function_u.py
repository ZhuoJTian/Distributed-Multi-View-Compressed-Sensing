...
def baseline_JSM1(A, Y, X, V, N, M_list, Adjacent_Matrix, c, rho, gamma1, gamma2, gamma3, MAX_ITER):
    # corrupted Lasso
    iter_num = 10000
    T = np.size(A, 1)
    e_e = np.zeros((T, N))
...
def decentral_l1_VR_penalty_c_ind_hard(A, Y, X, V, N, M_list, Adjacent_Matrix, alpha, c, gamma, MAX_ITER, eta, USTp, Umin):
    iter_num = 10000
    T = np.size(A, 1)
    x_e = np.zeros((T, N))
    v_e = np.ones((T, N))
...
def ADMM_VPD_JSM1_ind(A, Y, X, V, N, M_list, Adjacent_Matrix, alpha, c,
...
def stand_DLasso(A, Y, X, V, N, M_list, Adjacent_Matrix, c, gamma, MAX_ITER):
    # corrupted Lasso
    QUIET = 0
    iter_num = 10000
    m = np.size(A, 0)
